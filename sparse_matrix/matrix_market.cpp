...
void read_mm(const char *fname, Mat& A)
{
    FILE *f;
    MM_typecode matcode;
    int M, N, nz;
    ...
    A.rowptr.resize(A.n_rows+1);
    A.col_idx.resize(A.nnz);
    A.data.resize(A.nnz);

    std::vector<int> p(A.nnz);
    std::vector<bool> done(A.nnz, false);
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        while (i != k)
        {
            std::swap(rows[prev_k], rows[k]);
            std::swap(cols[prev_k], cols[k]);
